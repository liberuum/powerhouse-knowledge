...
tools:
  ...
  - Agent
---
<!-- GENERATED from AGENT.md (sha256:e001e9bfc7df9743) by scripts/build-agent.mjs — edit AGENT.md, not this file -->
...
1. **Two-batch pattern**: content ops (SET_TITLE, SET_DESCRIPTION, SET_NOTE_TYPE, SET_CONTENT, ADD_TOPIC) in one `docs apply`; `SET_PROVENANCE` in a separate call. A validation failure rejects the whole batch it is in.
2. **Description max 200 chars**: longer descriptions fail silently and take the batch with them.
3. **Always verify after creating**: `switchboard docs tree <drive> --format json` to confirm the node exists. CLI bugs and network blips cause silent failures.
4. **Dispatch dependent operations one at a time.** `docs apply` preserves order (verified on CLI 1.0.32), but a validation failure anywhere in a batch rejects the whole batch — so pipeline ops (ADD_TASK → ASSIGN_TASK → ADVANCE_PHASE), which each validate against the previous one's result, go one per `docs mutate` call with a read-back between them.
